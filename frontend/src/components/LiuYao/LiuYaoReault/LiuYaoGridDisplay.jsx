/*
 * @file            frontend/src/components/LiuYao/LiuYaoReault/LiuYaoGridDisplay.jsx
...
const LiuYaoGridDisplay = React.memo(({ divinationData }) => {
  ...
  try {
            ...
            {divinationData.ben_gua_body && divinationData.ben_gua_body.map((yao, index) => {
              ...
              return (
                <div key={`ben-yao-${index}`} className={`grid-row position-${yao.position}`}>
                  ...
                  {hasDongYao && (
                    <div className="grid-cell bian-body-cell">
                      {bianYao && (
                        <>
    ...
    return (
      <div className="liu-yao-error" role="alert" aria-label="六爻排盘加载失败">
        六爻排盘加载失败，请稍后重试
      </div>
    );
  }
