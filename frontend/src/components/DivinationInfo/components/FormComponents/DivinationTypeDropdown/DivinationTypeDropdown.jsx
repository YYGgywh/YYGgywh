/*
 * @file            frontend/src/components/DivinationInfo/components/FormComponents/DivinationTypeDropdown/DivinationTypeDropdown.jsx
 * @description     占类下拉选择组件，提供财富、职业、婚恋等占类选项
 * @author          Gordon <gordon_cao@qq.com>
 * @createTime      2026-02-18 10:30:00
 * @lastModified    2026-03-09 20:30:00
...
// 导入组件样式文件
import styles from './DivinationTypeDropdown.desktop.module.css';
...
const DivinationTypeDropdown = ({ selectedType, onTypeSelect }) => {
  ...
  // 渲染占类下拉选择组件
  return (
    <div className={styles.divinationTypeContainer}> {/* 占类下拉选择组件容器 */}
      <label className={styles.typeLabel}></label> {/* 占类下拉选择组件标签 */}
      <div className={styles.dropdownContainer} ref={dropdownRef}> {/* 占类下拉选择组件容器 */}
        <div className={styles.dropdownSelect} onClick={handleToggle} tabIndex="0"> {/* 占类下拉选择组件选择框，tabIndex 使其可聚焦 */}
          {selectedType || '占类'} {/* 占类下拉选择组件选择框文本 */}
          <span className={styles.dropdownArrow}>▼</span> {/* 占类下拉选择组件选择框下拉箭头 */}
        </div>
        {/* 占类下拉选择组件菜单 */}
        {isOpen && (
          <div className={styles.dropdownMenu}> {/* 占类下拉选择组件菜单 */}
            {/* 占类下拉选择组件菜单选项 */}
            {divinationTypes.map((type) => (
              // 占类下拉选择组件菜单选项
              <div
                key={type} // 占类下拉选择组件菜单选项键值
                className={styles.dropdownItem} // 占类下拉选择组件菜单选项类名
