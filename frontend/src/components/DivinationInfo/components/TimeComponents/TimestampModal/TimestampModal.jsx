/*
 * @file            frontend/src/components/DivinationInfo/components/TimeComponents/TimestampModal/TimestampModal.jsx
 * @description     时间戳设置弹窗主组件，管理选项卡状态和协调三个时间组件
 * @author          圆运阁古易文化 <gordon_cao@qq.com>
 * @createTime      2026-01-30 11:00:00
 * @lastModified    2026-03-09 21:21:35
      ...
      <div className={`${styles.modalContent} ${styles.timestampModal}`}> {/* 模态框内容容器 */}
        <div className={styles.modalHeader}> {/* 模态框头部区域 */}
          <div className={styles.timestampTabs}> {/* 选项卡容器 */}
            {/* 公历选项卡按钮 */}
            <button
              className={`${styles.tabButton} ${activeTab === 'gregorian' ? styles.active : ''}`} // 根据当前选项卡设置激活样式
            ...
            {/* 农历选项卡按钮 */}
            <button
              className={`${styles.tabButton} ${activeTab === 'lunar' ? styles.active : ''}`} // 根据当前选项卡设置激活样式
            ...
            {/* 四柱选项卡按钮 */}
            <button
              className={`${styles.tabButton} ${activeTab === 'four-pillars' ? styles.active : ''}`} // 根据当前选项卡设置激活样式
        ...
        {/* 时间输入区域 */}
        <div className={`${styles.modalBody} ${styles.timestampModalBody}`}> {/* 模态框主体区域 */}
          {/* 如果当前是公历选项卡 */}
          {activeTab === 'gregorian' && (
            // 渲染公历时间组件
            <SolarTime
          ...
          {activeTab === 'four-pillars' && (
            <> {/* Fragment标签，用于包裹多个元素 */}
              ...
              {/* 四柱计算结果显示区域 */}
              {solarListResult && (
                <div className={styles.solarListResult}> {/* 四柱结果列表容器 */}
                  {/* 如果计算成功 */}
                  {solarListResult.success ? (
                    <div className={styles.resultList}> {/* 结果列表容器 */}
                      {/* 反转结果列表并遍历 */}
                      {solarListResult.data.list.slice().reverse().map((solar, index) => {
                        ...
                        return (
                          ...
                          <div
                            ...
                            className={`${styles.resultItem} ${isSelected ? styles.selected : ''}`} // 根据选中状态设置样式
                            onClick={() => handleResultItemClick(solar, index)} // 点击处理函数
                          >
                            <span className={styles.resultIndex}>{formattedIndex}/{formattedTotal}</span> {/* 显示索引和总数 */}
                            <span className={styles.resultContent}>{solar.fullString}</span> {/* 显示完整时间字符串 */}
                          </div> // 结果项容器结束
                        ); // return结束
                      })} {/* map遍历结束 */}
                    </div> // 结果列表容器结束
                    ...
                    // 如果计算失败
                    <div className={styles.resultError}> {/* 错误信息容器 */}
