/*
 * @file            frontend/src/components/Waterfall/WaterfallContainer.jsx
...
const WaterfallContainer = ({ 
  ...
  // 返回瀑布流容器的JSX结构
  return (
    <div className="waterfall-container">  {/* 瀑布流容器外层，控制整体布局 */}
      <div className="waterfall-grid">  {/* 瀑布流网格，使用CSS Grid或Flex布局实现瀑布流效果 */}
        {/* 遍历数据列表，渲染每个瀑布流卡片 */}
        {items.map((item) => (
          <WaterfallCard
        ...
        {hasMore && (  // 条件渲染：只有当还有更多数据时才显示触发点
          <div className="load-more-trigger" ref={loadMoreRef}>  {/* 滚动触发器，绑定ref用于监听滚动位置 */}
