# backend/src/algorithms/liuyao_calculator.py 2025-11-30 21:20:17
# 功能：六爻占卜核心算法实现
...
def get_day_gan_from_calendar():
    ...
    try:
        ...
        now = datetime.now()
        current_solar = Solar.fromDate(now)
        year = current_solar.getYear()
        month = current_solar.getMonth()
        day = current_solar.getDay()
...
def test_liuyao_integration():
    ...
    now = datetime.now()
    current_solar = Solar.fromDate(now)
    print(f"当前时间: {current_solar.toYmd()}")
